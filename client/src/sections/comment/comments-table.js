    ...
    <Card>
      <Scrollbar>
        <Box sx={{ minWidth: 800 }}>
          ...
          <Table>
            <TableHead>
              <TableRow>
                <TableCell>{t("comments.branch")}</TableCell>
                <TableCell>{t("comments.rate")}</TableCell>
                <TableCell>{t("comments.comment")}</TableCell>
                <TableCell>{t("comments.date")}</TableCell>
                <TableCell>{t("comments.actions")}</TableCell>
              </TableRow>
            </TableHead>
            <TableBody>
              {items &&
                items.map((comment) => {
                  ...
                  return (
                    <TableRow hover key={comment.id} selected={isSelected}>
                      <TableCell>{comment?.restaurant?.name ?? "-"}</TableCell>
                      <TableCell>
      ...
      <TablePagination
        ...
        labelDisplayedRows={({ from, to, count }) =>
          t("comments.displayedRows", { from: from, to: to, count, count })
        }
      ...
      <ConfirmModal
        ...
        description={t("comments.deleteConfirmModalDescription", {
          ...
          try {
            const response = await fetch(
              process.env.NEXT_PUBLIC_BACKEND_SERVER +
                `/comment/${selectedForDelete?._id}/deleteComment`,
              {
                method: "DELETE",
                headers: {
