...
export const withAuthGuard = (Component) => (props) => {
  ...
  if(router?.pathname.startsWith("/branchMenu") || (!router?.pathname.startsWith("/branchMenu") && items.filter(item => item.path === router.pathname)?.[0].permission.includes(state?.user?.user?.role))){
    return(
      <AuthGuard>
        <Component {...props} />
      </AuthGuard>
